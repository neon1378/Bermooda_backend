...
class CoreWebSocket(AsyncJsonWebsocketConsumer):
    """
    Optimized WebSocket consumer for real-time workspace communication
    """
    ...
    async def connect(self):
        ...
        self.workspace_obj = await sync_to_async(self._get_current_workspace)()
        await self.change_user_online(True)

        unread_count = await self._get_all_group_unread_messages()
        await self.send_initial_data(unread_count)

    @sync_to_async
    def _get_current_workspace(self):
        return UserAccount.objects.select_related('current_workspace') \
            .get(id=self.user.id).current_workspace

    async def send_initial_data(self, unread_count):
        await self.send_json({
            "data_type": "all_group_message_unread",
            "data": {"message_count": unread_count}
        })

    async def disconnect(self, close_code):
        await self.change_user_online(False)
        await self.channel_layer.group_discard(
            self.user_group_name,
            self.channel_name
        )
    ...
    @sync_to_async
    def change_user_online(self, status):
        UserAccount.objects.filter(id=self.user.id).update(is_online=status)
    ...
    async def get_group_messages_handler(self, data):
        data = await self._group_message_serialize()
        await self.send_json({"data_type": "group_messages", "data": data})

    @sync_to_async
    def _group_message_serialize(self):
        groups = GroupMessage.objects.filter(
            workspace=self.workspace_obj,
            members=self.user
        ).prefetch_related(
            'members',
            Prefetch('group_text_messages',
                     queryset=TextMessage.objects.order_by('-created_at'))
        )
        ...
        serializer = GroupSerializer(sorted_groups, many=True, context={'user': self.user})
        data = serializer.data

        for group in data:
            other_member = next(
                (m for m in group['members'] if m['id'] != self.user.id),
                {}
            )
            group.update({
                'fullname': other_member.get('fullname'),
                'avatar_url': other_member.get('avatar_url'),
                'is_online': other_member.get('is_online')
            })
    ...
    async def new_message_handler(self, data):
        ...
        group_name = f"group_message{group_id}"
        result = await self._send_new_message(data)
    ...
    def _send_new_message(self, data):
        ...
        return {"status": True, "data": {"message_id": message.id}}

    async def group_message_update(self, event):
        message = await sync_to_async(TextMessage.objects.get)(id=event["message_id"])
        if message.owner != self.user:
            message.is_read = True
            await sync_to_async(message.save)()

        serializer = await sync_to_async(TextMessageSerializer)(message)
        data = serializer.data
        data["self"] = event["owner_id"] == self.user.id
    ...
    @sync_to_async
    def _get_group_message_list(self, page_number, group_id):
        group = GroupMessage.objects.get(id=group_id)
        group.group_text_messages.filter(
            is_read=False
        ).exclude(
            owner=self.user
        ).update(is_read=True)

        messages = group.group_text_messages.order_by("-id")
        ...
        return {
            ...
            "next": page.next_page_number() if page.has_next() else None,
            "previous": page.previous_page_number() if page.has_previous() else None,
            "list": TextMessageSerializer(page.object_list, many=True).data
        }
    ...
    async def change_current_workspace(self, event):
        ...
        await self.send_workspace_data()

    async def send_workspace_data(self):
        current_workspace = await self._get_workspace_data(self.workspace_id)
        await self.send(json.dumps({
            "data_type": "change_current_workspace",
    ...
    @sync_to_async
    def _get_workspace_data(self, workspace_id):
        workspace = WorkSpace.objects.select_related('wallet').get(id=workspace_id)
        return {
            "wallet": {
                "id": workspace.wallet.id if workspace.wallet else None,
                "balance": int(workspace.wallet.balance) if workspace.wallet else 0
            },
            "id": workspace.id,
    ...
    async def handle_task_list(self, data):
        project_id = data.get("project_id")
        data = await self._main_serializer_data(project_id)
        await self.send_json({"data_type": "task_list", "data": data})

    @sync_to_async
    def _main_serializer_data(self, project_id):
        categories = CategoryProject.objects.filter(
            project_id=project_id
        ).order_by("-id").values('id', 'color_code', 'title')

        tasks = self._get_filtered_tasks(project_id)
        ...
        for task in task_data:
            cid = task['category_task_id']
            if cid in category_map:
                category_map[cid]['task_list'].append(task)

        return sorted(category_map.values(), key=lambda x: x['id'])
    ...
    def _get_filtered_tasks(self, project_id):
        base_qs = Task.objects.filter(
            project_id=project_id,
            done_status=False
        ).select_related('category_task').prefetch_related('check_list')

        if self._has_admin_access():
            return base_qs
        return base_qs.filter(check_list__responsible_for_doing=self.user).distinct()
    ...
    async def customer_list_handler(self, data):
        group_crm_id = data.get("group_crm_id")
        customer_data = await self._customer_list_serializer(group_crm_id)
        await self.send_json({"data_type": "customer_list", "data": customer_data})
    ...
    def _customer_list_serializer(self, group_crm_id):
        labels = Label.objects.filter(
            ...
            Prefetch('label_step__steps',
                     queryset=LabelStep.objects.order_by('step'))
        ).values('id', 'color', 'title')

        customers = CustomerUser.objects.filter(
            group_crm_id=group_crm_id,
            is_followed=False
        ).select_related('label').order_by('order')
        ...
        for label in labels:
            result.append({
                ...
                "color": label['color'],
                "title": label['title'],
                "steps": LabelStepSerializer(
                    LabelStep.objects.filter(label_step__label=label['id']),
                    many=True
                ).data,
                "group_crm_id": group_crm_id,
                "customer_list": []
        ...
        for customer in customers:
            label_entry = next(
                (r for r in result if r['label_id'] == customer.label.id),
                None
            )
            if label_entry:
                label_entry['customer_list'].append(
                    CustomerSmallSerializer(customer).data
                )

        return sorted(result, key=lambda x: x["label_id"])
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if handler := handler_map.get(command):
            await self._setup_groups(command, data)
            await handler(data)
        else:
            await self.send_error("Invalid command")

    async def _setup_groups(self, command, data):
        if command == "task_list":
            await self._add_project_group(data)
        elif command == "customer_list":
            await self._add_crm_group(data)
        elif command == "read_message_list":
            await self._add_message_group(data)

    async def _add_project_group(self, data):
        self.project_group_name = f"{data.get('project_id')}_gp_project"
        await self.channel_layer.group_add(
            self.project_group_name,
            self.channel_name
        )

    async def _add_crm_group(self, data):
        self.crm_group_name = f"{data.get('group_crm_id')}_gp_group_crm"
        await self.channel_layer.group_add(
            self.crm_group_name,
            self.channel_name
        )

    async def _add_message_group(self, data):
        group_name = f"group_message{data.get('group_id')}"
    ...
    # Additional handlers for other commands can be added following the same pattern